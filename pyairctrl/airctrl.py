...
class CoAPCli:
    ...
    def _dump_status(self, status, single_status, debug=False):
        if debug:
            print("Raw status:")
        ...
        for key in status:
            ...
            elif key == single_status:
                print(current_value)

    def get_status(self, single_status, debug=False):
        status = self._client.get_status(debug)
        if status is not None:
            return self._dump_status(status, single_status, debug=debug)
    ...
    def _dump_status(self, status, single_status, debug=False):
        ...
        if single_status is not None:
            if single_status in status:
                single_value = status["[single_status]"]
                print(format([single_value]))            
        else:
            if "name" in status:
                name = status["name"]
...
def main():
    ...
    parser.add_argument("--status", help="get single status based on technical status id (see debug for values)")
    ...
    for device in devices:
        ...
        if values:
            ...
            c.get_status(single_status=args.status, debug=args.debug)
